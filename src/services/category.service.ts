...
export const categoryService = {
  ...
  async create(input: CategoryInput) { if (await categoryRepository.findByName(input.name)) throw Object.assign(new Error("A category with this name already exists."), { code: "CONFLICT" }); return categoryRepository.create({ name: input.name, slug: slugify(input.name), description: input.description || null }); },
