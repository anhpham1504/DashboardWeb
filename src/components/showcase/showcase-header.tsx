...
export function ShowcaseHeader() {
  ...
  useEffect(() => {
    const followLegacyLink = () => {
      if (pathname === "/" && window.location.hash === "#about") router.replace("/about");
    };
    followLegacyLink();
    ...
    <header className={styles.header}>
      <div className={styles.headerInner}>
        ...
        <div className={styles.headerActions}>
          <ThemeToggle />
          <Link href="/#projects" className={styles.headerCta}>Khám phá <ArrowUpRight size={16} /></Link>
          <button className={styles.menuButton} onClick={() => setOpen(true)} aria-label="Mở menu điều hướng"><Menu size={22} /></button>
        </div>
      </div>
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="right" className="w-[min(340px,100vw)]">
          <SheetHeader><SheetTitle>FPT Digital Showcase</SheetTitle></SheetHeader>
          <nav className={styles.mobileNav} aria-label="Điều hướng trên điện thoại">
            {links.map(link => <Link key={link.href} href={link.href} aria-current={pathname === link.href ? "page" : undefined} onClick={() => setOpen(false)}>{link.label}<ArrowUpRight size={18} /></Link>)}
          </nav>
        </SheetContent>
      </Sheet>
    </header>
